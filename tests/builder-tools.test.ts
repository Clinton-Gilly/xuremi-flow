...
const {
  ...
  toolCallLabel,
} = await import("@/lib/builder-protocol");
...
describe("the Builder's session attributes", () => {
  ...
  it("exports connection and model request headers", () => {
    expect(BUILDER_CONNECTION_HEADER).toBe("x-xuremi-flow-connection");
    expect(BUILDER_MODEL_HEADER).toBe("x-xuremi-flow-model");
  });
});
    ...
    toolMetadata: {
      eve: {
        ...
        inputRequest: {
          requestId: "req_1",
          kind: "question",
          prompt: "Xuremi Flow needs a Notion connection.",
