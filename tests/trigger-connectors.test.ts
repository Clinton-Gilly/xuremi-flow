...
const GET_ME_OK = { body: { ok: true, result: { id: 123456789, is_bot: true, username: "xuremiflow_bot" } } };
...
describe("telegram connector", () => {
  ...
  it("validates a token with getMe and captures the bot identity", async () => {
    ...
    expect(result.label).toBe("@xuremiflow_bot");
    expect(result.hint).toBe(TOKEN.slice(-4));
    expect(result.meta).toEqual({ bot_username: "xuremiflow_bot", bot_id: 123456789 });
  });
  ...
  it("registers the per-connection webhook with a generated secret token", async () => {
    ...
    const extra = await telegramConnector.afterCreate!({
      ...
      appOrigin: "https://xuremiflow.vercel.app",
    });
    ...
    const inboundUrl = `https://xuremiflow.vercel.app/api/events/telegram/${CONNECTION}`;
  ...
  it("generates a different secret token for every connection", async () => {
    ...
    const args = { connectionId: CONNECTION, secret: { botToken: TOKEN }, appOrigin: "https://xuremiflow.vercel.app" };
...
describe("stripe connector", () => {
  it("is a free payments signing-secret connector with one secret field", () => {
    ...
    expect(stripeConnector.fields[0]).toMatchObject({
      ...
      kind: "secret",
      placeholder: "whsec_…",
      help: "From the Stripe webhook endpoint you point at Xuremi Flow",
    });
  });
  ...
  it("hands back the inbound URL to paste into Stripe, and no new secret", async () => {
    ...
    expect(extra.meta).toEqual({ inboundUrl: `https://xuremiflow.vercel.app/api/events/stripe/${CONNECTION}` });
  });
});
