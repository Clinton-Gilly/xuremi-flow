...
const HERE = "https://xuremiflow.test/w/abc123";
...
describe("isInternalHref", () => {
  it("accepts pages of this app", () => {
    ...
    expect(isInternalHref("https://xuremiflow.test/connections", HERE)).toBe(true);
  ...
  it("rejects anywhere this app cannot save first", () => {
    ...
    expect(isInternalHref("mailto:sonny@xuremiflow.test", HERE)).toBe(false);
    expect(isInternalHref("tel:+441234567890", HERE)).toBe(false);
    expect(isInternalHref("javascript:void(0)", HERE)).toBe(false);
    expect(isInternalHref("blob:https://xuremiflow.test/9f2", HERE)).toBe(false);
  });
...
describe("shouldGuardClick", () => {
  ...
  it("leaves external links to the browser's own prompt", () => {
    ...
    expect(shouldGuardClick(click("mailto:sonny@xuremiflow.test"), HERE)).toBe(false);
  });
