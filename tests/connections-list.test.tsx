...
describe("inboundFor", () => {
  ...
  it("uses the URL the connector registered, and says whether Telegram accepted it", () => {
    const url = "https://xuremiflow.test/api/events/telegram/cn_1";
  ...
  it("tells Stripe's owner where to paste it", () => {
    const url = "https://xuremiflow.test/api/events/stripe/cn_1";
  ...
  it("warns a Resend account with no verified domain, and only that one", () => {
    ...
    expect(
      noticeFor(
        connection({
          provider: "resend",
          meta: { domains: [{ name: "xuremiflow.test", status: "verified" }] },
        }),
      ),
    ).toBeNull();
  });
