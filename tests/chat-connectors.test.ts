...
const USERS_BODY = {
  ...
  members: [
    ...
    { id: "U3", name: "grace" },
    { id: "UBOT", name: "xuremiflow", is_bot: true },
...
describe("discord webhook connector", () => {
  ...
  it("GETs the webhook itself and captures the channel it posts to", async () => {
    ...
    expect(result.label).toBe("#xuremiflow");
...
describe("discord bot connector", () => {
  ...
  it("identifies the bot with a Bot token and builds the invite URL", async () => {
    const calls = stubFetch({ [USERS_ME]: { body: { id: "B77", username: "xuremiflow" } } });
...
describe("slack app manifest", () => {
  ...
  it("switches interactivity on and points it at this deployment's own Slack endpoint", () => {
    ...
    const substituted = substituteAppOrigin(
      ...
      "https://xuremiflow.vercel.app",
    );
    expect(substituted).toBe("https://xuremiflow.vercel.app/api/events/slack");
  ...
  it("names the app and derives a bot display name Slack will accept", () => {
    expect(manifest().display_information.name).toBe("Xuremi Flow");
    expect(manifest().features.bot_user.display_name).toBe("Xuremi-Flow");
    ...
    expect((slackAppManifest("   ") as unknown as Manifest).display_information.name).toBe("Xuremi Flow");
  });
