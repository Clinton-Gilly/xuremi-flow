...
/** What every tool must answer with when Xuremi Flow's backend is not configured. */
...
describe("the Builder's tool guard", () => {
  it("returns a terminal result rather than throwing, so the model stops", async () => {
    ...
    expect(failure).toMatchObject({
      ...
      retryable: false,
      message: "The Builder cannot reach Xuremi Flow's backend: builder-engine: CONVEX_URL is not set.",
    });
  ...
  it("describes a thrown non-error without inventing a cause", () => {
    expect(serviceUnavailable(undefined).message).toBe(
      "The Builder cannot reach Xuremi Flow's backend: the backend could not be reached.",
    );
  });
