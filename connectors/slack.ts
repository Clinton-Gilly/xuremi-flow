...
const DEFAULT_APP_NAME = "Xuremi Flow";
...
export function slackAppManifest(appName: string = DEFAULT_APP_NAME): Record<string, unknown> {
  ...
  return {
    display_information: {
      name,
      description:
        "Runs your Xuremi Flow workflows: posts messages and asks for approvals, in channels or DMs.",
    },
    features: {
...
export const slackConnector = defineConnector({
  ...
  setup: {
    ...
    steps: [
      ...
      "Invite the bot to any private channel you want to post in (/invite @Xuremi Flow). Public channels work without an invite.",
  ...
  emptyHint(kind) {
    ...
    return (
      "No channels or people yet. Invite the bot to a channel (/invite @Xuremi Flow), or pick a " +
