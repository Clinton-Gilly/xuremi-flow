...
function convexErrorData(error: unknown): Record<string, unknown> | null {
  ...
  const duckData = (error as { data?: unknown }).data;
  if (typeof duckData === "object" && duckData !== null && !Array.isArray(duckData)) {
    return duckData as Record<string, unknown>;
  }
  const message = (error as { message?: unknown }).message;
  if (typeof message === "string" && message.includes("version_conflict")) {
    const match = message.match(/\{.*"code"\s*:\s*"version_conflict".*\}/);
    if (match) {
      try {
...
export function Canvas({
  ...
  const save = useCallback((): Promise<boolean> => {
    ...
    // If the server has already moved ahead of local version (e.g. Builder updates or another tab),
    // prompt conflict resolution directly rather than firing a mutation guaranteed to fail with a
    // server-side ConvexError (which triggers Next.js Turbopack console error overlay).
    if (workflowRef.current.version > versionRef.current) {
      setSaveState("conflict");
      setConflict({ version: workflowRef.current.version });
  ...
  useEffect(() => {
    ...
    if (nodes.length === 0 || !dirtyRef.current || reloadRef.current) {
      adopt(workflow);
      return;
    }
              ...
              */}
              {remote ? (
                <Panel position="top-center" className="mt-2">
                  <div
                    role="status"
                    className="flex max-w-lg items-center gap-3 rounded-lg border border-border bg-card/95 px-3 py-2 text-sm shadow-sm backdrop-blur-sm"
                  >
                    <span>
                      {remote.fromBuilder
                        ? "The Builder updated this workflow"
                        : "This workflow changed elsewhere"}
                    </span>
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={reloadTheirs}>
                        {remote.fromBuilder ? "View Builder's" : "Reload theirs"}
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => {
                          const expected = Math.max(remote.version, workflowRef.current.version);
                          versionRef.current = expected;
                          void commit(expected);
                        }}
                      >
                        Keep mine
                      </Button>
