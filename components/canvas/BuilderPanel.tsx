...
import { modelsFromMeta, pickModelId, PREFERRED_PROVIDERS } from "@/agents/builder/lib/models";
import {
  BUILDER_CONNECTION_HEADER,
  BUILDER_FEATURE,
  BUILDER_MODEL_HEADER,
...
function getLatestTurnFailure(
  ...
  for (let index = events.length - 1; index >= 0; index -= 1) {
    ...
    if (event.type === "turn.failed") {
      const data = (event as { data?: { code?: string; message?: string } }).data;
...
function BuilderModelSelector({
  ...
  const [customModelDraft, setCustomModelDraft] = useState(modelId);

  const aiConnections = useMemo(() => {
    return (connections ?? []).filter(
      (conn) => conn.status === "active" && acceptsConnection("ai")(conn),
    );
  }, [connections]);

  const activeConnection = useMemo(() => {
    if (connectionId === "house") return null;
    return aiConnections.find((c) => c._id === connectionId);
  }, [aiConnections, connectionId]);

  const availableModels = useMemo(() => {
    if (connectionId === "house") {
      return HOUSE_MODELS.map((m) => ({ id: m.id, label: m.label }));
    }
    if (activeConnection) {
      const models = modelsFromMeta(activeConnection.meta);
  ...
  return (
    <div className="border-b border-border bg-muted/20 px-3 py-2 text-xs">
      ...
      <div className="grid grid-cols-2 gap-1.5">
        ...
        <Select
          value={connectionId}
          disabled={disabled || connections === undefined}
          onValueChange={(next) => {
            if (typeof next !== "string") return;
            onConnectionChange(next);
            if (next === "house") {
              onModelChange("openai/gpt-5.6-luna");
            } else {
              const selectedConn = aiConnections.find((c) => c._id === next);
              if (selectedConn) {
                const models = modelsFromMeta(selectedConn.meta);
                const preference = PREFERRED_PROVIDERS.find(
                  (p) => p.provider === selectedConn.provider,
                );
                const defaultModel =
                  pickModelId(models, preference?.prefer ?? []) || models[0] || "";
        ...
        >
          <SelectTrigger size="sm" className="w-full text-xs h-7">
            <SelectValue>
              {(current: unknown) => {
                if (current === "house" || !current) return "Platform Default";
                const match = aiConnections.find((c) => c._id === current);
                return match ? `${match.label} ••••${match.hint}` : "Provider";
              }}
            </SelectValue>
          </SelectTrigger>
        ...
        {isTypingModel ? (
          ...
          >
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key === "Escape") setIsTypingModel(false);
              }}
            />
            <Button
              type="submit"
          ...
          <div className="flex items-center gap-1 min-w-0">
            ...
            >
              <SelectTrigger size="sm" className="w-full text-xs h-7 flex-1 min-w-0">
                <SelectValue>
                  {(current: unknown) => {
                    if (typeof current !== "string" || !current) {
                      return connectionId === "house" ? "GPT-5.6 Luna" : "Select model…";
                    }
                    const match = availableModels.find((m) => m.id === current);
            ...
            <Button
              ...
              title="Type custom model ID"
              onClick={() => {
                setCustomModelDraft(modelId);
                setIsTypingModel(true);
              }}
            >
...
function BuilderChat({
  ...
  const [selectedConnectionId, setSelectedConnectionId] = useState<string>(() => {
    ...
    return localStorage.getItem("papaflow:builder:connection") || "house";
  });
  const [selectedModelId, setSelectedModelId] = useState<string>(() => {
    if (typeof window === "undefined") return "";
    return localStorage.getItem("papaflow:builder:model") || "";
  });

  const handleConnectionChange = useCallback((nextId: string) => {
    setSelectedConnectionId(nextId);
    if (typeof window !== "undefined") {
      localStorage.setItem("papaflow:builder:connection", nextId);
    }
  }, []);
  ...
  const handleModelChange = useCallback((nextModel: string) => {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("papaflow:builder:model", nextModel);
    }
  }, []);
  ...
  const getHeadersRef = useRef<() => Record<string, string>>(() => ({
    [BUILDER_WORKFLOW_HEADER]: workflowId,
  }));
  useEffect(() => {
    getHeadersRef.current = () => {
      ...
      if (selectedConnectionId) {
        headers[BUILDER_CONNECTION_HEADER] = selectedConnectionId;
      }
      if (selectedModelId) {
        headers[BUILDER_MODEL_HEADER] = selectedModelId;
      }
      return headers;
    };
  }, [workflowId, selectedConnectionId, selectedModelId]);
  ...
  const busy = (agent.status === "submitted" || agent.status === "streaming") && !agent.error;
  ...
  const turnFailure = busy || resuming ? undefined : getLatestTurnFailure(agent.events);
  const errorMessage = agent.error?.message ?? turnFailure;
      ...
      >
        ...
        <div className="mt-2 flex items-center gap-2">
          ...
          {busy ? (
            <>
              <Button
                ...
                variant="ghost"
                onClick={() => {
                  void agent.cancel().catch((cause: unknown) => console.error(cause));
                  if (sessionId && retiredRef.current !== sessionId) {
                    void retire(sessionId, "The user stopped the builder chat.");
                  }
                  agent.reset();
                }}
              >
            ...
            <Button
              ...
              disabled={resuming || (messages.length === 0 && !errorMessage)}
              onClick={() => {
                if (sessionId && retiredRef.current !== sessionId) {
                  void retire(sessionId, "The user started a new builder chat.");
                }
          ...
          <span role="status" className="ml-auto text-xs text-muted-foreground">
            {resuming ? "Catching up…" : busy ? "Working" : errorMessage ? "Failed" : finished ? "Finished" : "Ready"}
          </span>
        </div>
      </form>
    </PanelShell>
