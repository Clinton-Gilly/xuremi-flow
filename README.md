# Xuremi Flow — AI-Native Workflow Automation SaaS with Clerk, Convex, Vercel Workflows & eve
...
> **Disclaimer:** Xuremi Flow is a fictional educational project. The organisations,
...
Xuremi Flow is an **n8n-style workflow automation platform built as a multi-tenant
...
![The Xuremi Flow canvas — a workflow with a Manual trigger, an HTTP request and an LLM node that still needs a connection, with the last run's timeline underneath](docs/assets/canvas.png)
...
> Most workflow-engine demos stop at "nodes execute in order". Xuremi Flow rebuilds
...
Xuremi Flow is three products fused together: a visual automation editor, a durable
...
async execute({ provider, reason }, ctx) {
  "use workflow";
  const pending = ask(ctx, { prompt: `Xuremi Flow needs a ${providerName} connection. ${reason}`, display: "confirmation", allowFreeform: true, options: [...] });
...
```bash
clerk apps create "Xuremi Flow" --json        # note the application_id
...
→ Xuremi Flow (Development) → **Activate Convex integration**. Copy the Frontend API
URL it shows, then:
...
1. **Connections → Add connection** — paste an OpenAI, Anthropic, Google or Groq
   key. Xuremi Flow calls the provider's list-models endpoint, seals the key and
...
This repository is for educational and reference purposes. Xuremi Flow is a
