...
const SHARED_SECRET_REJECTED =
  "Convex rejected Xuremi Flow's shared secret — ENGINE_SECRET on this service and on the Convex " +
...
export function serviceUnavailable(error: unknown): ToolFailure {
  return {
    ok: false,
    error: "service_unavailable",
    message: `The Builder cannot reach Xuremi Flow's backend: ${messageOf(error)}.`,
    retryable: false,
  };
}
