...
import { modelsFromMeta, pickConnection, pickModelId, PREFERRED_PROVIDERS } from "./lib/models";
...
/** Context window token estimate per model identifier so Eve skips AI Gateway metadata lookup. */
function contextWindowForModel(modelId: string): number {
  const lower = modelId.toLowerCase();
  if (lower.includes("1m") || lower.includes("gemini")) return 1_000_000;
  if (lower.includes("claude") || lower.includes("sonnet") || lower.includes("opus")) return 200_000;
  return 128_000;
}
  ...
  model: defineDynamic({
    events: {
      "session.started": (_event, ctx) => {
        ...
        const effectiveModel = modelId || HOUSE_MODEL;
        return {
          model: effectiveModel,
          modelContextWindowTokens: contextWindowForModel(effectiveModel),
        };
      },
      ...
      "step.started": async (_event, ctx) => {
        ...
        const connectionId = attribute(attributes, "connectionId");
        const modelId = attribute(attributes, "modelId");
        ...
        try {
          // If the user explicitly requested an AI connection:
          if (connectionId && connectionId !== "house") {
            const opened = await openOrgConnection(connectionId, orgId);
            const apiKey = opened.secret.apiKey;
            if (typeof apiKey === "string" && apiKey) {
              const workspaceId = opened.secret.workspaceId;
              const metaModels = modelsFromMeta(opened.meta);
              const preference = PREFERRED_PROVIDERS.find((p) => p.provider === opened.provider);
              const effectiveModel =
                modelId ||
                pickModelId(metaModels, preference?.prefer ?? []) ||
                metaModels[0] ||
                "gpt-4o";

              const model = (await providerFor(opened.provider, apiKey, {
          ...
          // If connectionId is explicitly "house" (Platform Default):
          ...
          // Otherwise automatic selection from active org connections:
          ...
          if (choice) {
            ...
            if (typeof apiKey === "string" && apiKey) {
              ...
              const effectiveModel =
                modelId ||
                pickModelId(metaModels, choice.preference.prefer) ||
                metaModels[0] ||
                "gpt-4o";
